...
Functional tests belong with the systems they validate and should cover controlled failure behavior as well as successful behavior. Directorate content validation and the bounded vertical-slice smoke scenario run in `ForgeLine.Game.Tests` and must remain fully headless. Prototype battlefield validation, disruption/rerouting/restoration, finite-resource loading, and Command Core objective tests also run in `ForgeLine.Game.Tests` without a presentation dependency.
...
The interactive client composes the native Windows host, Direct3D 12 graphics backend, RTS input/camera stack, fixed-tick simulation, immutable presentation extraction, generic interpolated instances, development debug visualization, and the canonical Central Divide prototype battlefield. F2 includes the battlefield's starts, resources, strategic sites, logistics corridor, crossings, infrastructure state, and Command Core objectives alongside the existing system diagnostics.
