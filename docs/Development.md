...
Functional tests belong with the systems they validate and should cover controlled failure behavior as well as successful behavior. Directorate content validation and the bounded vertical-slice smoke scenario run in `ForgeLine.Game.Tests` and must remain fully headless.
